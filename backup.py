            ...
            if actual_price is not None:
                ...
                if live_btc_price_previous_day is not None:
                    ...
                    if historical_data:
                        ...
                        # Calculate RMSE
                        y_pred = model.predict(X_test)  # Prediction values from the model
                        ...
                        with st.expander(f'Root Mean Square Error (RMSE): {rmse:.2f} | R-squared (R²): {r2:.2f}'):
                            st.write(f"RMSE adalah metrik untuk mengukur tingkat kesalahan rata-rata antara prediksi dan data aktual. RMSE yang lebih rendah menunjukkan prediksi yang lebih akurat. Hasil RMSE: {rmse:.2f}.")
                        ...
                        # Membuat grafik batang untuk MAPE, RMSE, dan R²
                        fig_bar = px.bar(x=['MAPE', 'RMSE', 'R²'],
                                        y=[mape, rmse, r2],
                                        color=['MAPE', 'RMSE', 'R²'],
                                        labels={'y': 'Nilai'},
                                        title='Grafik Batang MAPE, RMSE, R²',
                                        barmode='group',
                                        width=400)
                        ...
                        fig_pie = px.pie(names=['MAPE', 'RMSE', 'R²'],
                                        values=[mape, rmse, r2],
                                        title='Grafik Lingkaran MAPE, RMSE, R²')

                        # Menampilkan grafik lingkaran pada Streamlit
